...
void ChatLogic::AddAllTokensToElement(std::string tokenID, tokenlist &tokens, T &element)
{
    ...
    while (true)
    {
        ...
        if (token != tokens.end())
        {
            element.AddToken(token->second); // add new keyword to edge or answer to node
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        {
                            //smart pointer (unique pointer to get exclusive owenrship)
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            //use move semantics to pass the ownsership from chatlogic to parent node
    ...
    //new code, chatlogic transfers the chatbot ownership to root node
